...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//div[@type='scene']">
        <xsl:variable name="scene"><xsl:value-of select="substring-after(head, 'SCENE ')"/></xsl:variable>
        <xsl:result-document href="{concat('/Users/chloe/Desktop/Don-Juan/', 'enscene', $scene, '.html')}">
            <html>
            <head>
            ...
            <body>
                <div id="{concat('en', head)}" class="tabcontent">
